...
int partition(int a[], int l, int r, int max_y, int max_x, data *qcsort, WINDOW *win) {
	...
	if(choice == 'p')
		return INT_MIN;
	print_numbers(qcsort, max_y, max_x);
	box_over_num(j, win, max_y, max_x, 3, 4, 3); /* height of box = 3, width of box = 4 */
	mvprintw(y_j, max_x / 4 + 11 + 5 * j, " ");
	attroff(A_BOLD);
	return j;
}
...
int quicksort(void) {
	...
	if(c == 'p') {
		...
		if(choice == 'p') {
			...
			instruction(max_y - 8, 0, "RED: Current position of pivot.");
			instruction(max_y - 7, 0, "GREEN: Final position of element.");
			sleep(3);
			...
			mvprintw(max_y - 10, max_x / 4 + 15, "Number of swaps = %d", qcsort.swaps);
			mvprintw(max_y - 8, max_x / 4 + 15, "Number of comparisons = %d", qcsort.comparisons);
		}
		curs_set(TRUE);
		move(max_y - 5, 0);
		clrtobot();
